...
#define EDGE_MAX_SUBCARRIERS  256   /**< Max subcarriers per frame. Sized for
                                          HT40 non-STBC (192 subcarriers,
                                          384 bytes/frame). STBC HT40
                                          (306 subcarriers / 612 bytes) still
                                          exceeds this and is dropped by
                                          process_csi_slot(); raise the cap
                                          if STBC frames become common. */
...
typedef struct {
    ...
    uint8_t  peer_id;                    /**< Source peer node_id (1-4). */
