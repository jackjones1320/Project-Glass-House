...
/*
 * High-rate mode (CONFIG_SAR_MODE=y): disables WiFi modem power-save so
 * the radio stays awake for every beacon interval. Required to hit the
 * 50 Hz CSI target — WIFI_PS_MIN_MODEM lets the radio sleep between
 * DTIMs and starves the CSI callback. Costs roughly +30-50% average
 * power draw versus MIN_MODEM.
 *
 * Kconfig booleans emit CONFIG_SAR_MODE=1 when 'y' and leave it
 * undefined when 'n', so `#if CONFIG_SAR_MODE` is the correct guard
 * (0 when undefined in preprocessor expressions).
 */
...
esp_err_t power_mgmt_init(uint8_t duty_cycle_pct)
{
     ...
     * PS affects CSI callback rate (receive-side wake windows); duty
     * cycle affects active/sleep scheduling of the application. They
     * are independent concerns. The set_ps call must run before the
     * duty_cycle==100 early-return so the policy still takes effect in
     * the default configuration. */
    wifi_ps_type_t ps_mode;
    const char *ps_label;
#if CONFIG_SAR_MODE
    ps_mode  = WIFI_PS_NONE;
    ps_label = "WIFI_PS_NONE (radio always on, +30-50% power draw)";
#else
    ...
    ESP_LOGI(TAG, "Power management initialized (SAR_MODE=%d)", CONFIG_SAR_MODE);
    return ESP_OK;
}
